...
public class EntityGen {
	...
	private final static String SQL_QUERY_ALL_TABLE_ORACLE = "SELECT * FROM USER_TABLES";
	...
	public static void genAllClass(String packagePath) throws Exception {
		...
		if (!CollectionUtils.isEmpty(list)) {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				String sql = "select * from user_tab_columns where Table_Name= '" + list.get(i) + "'";
				rs = con.createStatement().executeQuery(sql);
				while (rs.next()) {
					String colName = genEntityAttrname(rs.getString("COLUMN_NAME"));
					...
					if (dataType != null && !dataType.equals("")) {
						...
						if (dataType.contains("timestamp") || dataType.contains("TIMESTAMP")) {
							cb.setType("Date");
						}
						if (dataType.contains("NUMBER") || dataType.contains("number")) {
							cb.setType("Long");
						}
					}
	...
	public static List<String> queryAllTable() throws Exception {
		...
		ResultSet rs = con.createStatement().executeQuery(SQL_QUERY_ALL_TABLE_ORACLE);
		while (rs.next()) {
